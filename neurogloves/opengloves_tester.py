...
# Add this function after the imports but before main code

def patch_named_pipe():
    """Monkey patch the NamedPipe class to log sent data"""
    original_send = s.ipc.NamedPipe.send
    
    def send_with_logging(self, fingers, joys=None, bools=None):
        # Default values
        joys = joys or [0.0, 0.0]
        bools = bools or [False] * 8
        
        print("\n== Data Sending to Pipe ==")
        print(f"Fingers: {fingers}")
        print(f"Joystick: {joys}")
        print(f"Buttons: {bools}")
        
        # Create the binary representation (same as in the class)
        import struct
        data = struct.pack('@5f', *fingers)  # 5 floats for fingers
        data += struct.pack('@2f', *joys)    # 2 floats for joystick
        data += struct.pack('@8?', *bools)   # 8 booleans for buttons
        
        print(f"Binary data length: {len(data)} bytes")
        print(f"Hex representation: {data.hex()}")
        
        # Call the original function
        return original_send(self, fingers, joys, bools)
    
    # Replace the original send method with our logging version
    s.ipc.NamedPipe.send = send_with_logging
